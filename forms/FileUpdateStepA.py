...
from AMIC import FilesUpdateStepA, RJMessageBox
...
class FileUpdateStepA(Form):
    def __init__(self, oldFile: File):
        self.father: Form
        self.Form = FilesUpdateStepA()
        self.Form.cbxDepto.Items.Clear()
        ...
        self.Form.cbxAgencia.SelectedIndex = list(dictAgencias.values()).index(oldFile.agencia)
        self.Form.cbxDepto.SelectedIndex = list(dictBases.values()).index(oldFile.base)
        self.Form.cbxClasificacionHecho.SelectedIndex = list(dictClasificacionHecho.values()).index(oldFile.clasificacion_hecho)
        self.Form.cbxStatus.SelectedIndex = list(dictStatus.values()).index(oldFile.status)
        self.Form.cbxColaboracion.SelectedIndex = ["SI","NO"].index(oldFile.colaboracion)

        self.Form.txtNuc.Texts = oldFile.nuc
        self.Form.txtNoControl.Texts = oldFile.ncontrol
        self.Form.txtNoOficioCumplimiento.Texts = oldFile.no_oficio_cumplimiento

        date_fecha_emi = datetime.strptime(str(oldFile.fecha_emision),"%Y-%m-%d")
        date_fecha_cum = datetime.strptime(str(oldFile.fecha_cumplimentacion),"%Y-%m-%d")
        date_fecha_rec = datetime.strptime(str(oldFile.fecha_recibido),"%Y-%m-%d")

        self.Form.setFechaEmi(date_fecha_emi.year, date_fecha_emi.month, date_fecha_emi.day)
        self.Form.setFechaRec(date_fecha_rec.year, date_fecha_rec.month, date_fecha_rec.day)
        self.Form.setFechaCum(date_fecha_cum.year, date_fecha_cum.month, date_fecha_cum.day)
